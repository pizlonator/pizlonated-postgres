...
copyTemplateDependencies(Oid templateDbId, Oid newDbId)
{
	...
	while (HeapTupleIsValid(tup = systable_getnext(scan)))
	{
		...
		slot[slot_stored_count]->tts_values[Anum_pg_shdepend_objsubid - 1] = ObjectIdGetDatum(shdep->objsubid);
		...
		slot[slot_stored_count]->tts_values[Anum_pg_shdepend_deptype - 1] = ObjectIdGetDatum(shdep->deptype);
